...
class VisualSemanticEmbedding(nn.Module):
    def __init__(self, embed_ndim, device=torch.device('cpu')):
    ...
    def forward(self, img, txt):
        ...
        # text feature
        h0 = torch.zeros(1, img.shape[0], self.embed_ndim)
...
class Generator(nn.Module):
    def __init__(self, use_vgg=True, device=torch.device('cpu')):
        ...
        if use_vgg:
            ...
            self.encoder = \
                nn.Sequential(*(
                    self.encoder.features[i] for i in list(range(23)) + list(range(24, 33))))
    ...
    def forward(self, img, txt_feat, z=None):
        # encoder
        img_feat = self.encoder(img)  # img_feat: [batch, 3, 64, 64]

        z_mean = self.mu(txt_feat)
        z_log_stddev = self.log_sigma(txt_feat)
        z = torch.randn(txt_feat.shape[0], 128)
        ...
        txt_feat = txt_feat.repeat(1, 1, img_feat.shape[2], img_feat.shape[3])
